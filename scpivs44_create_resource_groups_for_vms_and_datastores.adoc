...
NOTE: Le système de stockage ASA r2 ne prend pas en charge le changement de nom des snapshots et, par conséquent, les fonctionnalités de renommage de SCV et de dénomination des snapshots récents ne sont pas prises en charge.
...
. Passez en revue le résumé, puis cliquez sur *Terminer*. À partir de SCV 6.1, les ressources de protection secondaire pour les systèmes ASA r2 sont visibles dans la page de résumé.
+

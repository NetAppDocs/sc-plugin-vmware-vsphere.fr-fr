...
+
Pour exclure globalement un ou plusieurs datastores des sauvegardes, vous devez spécifier le ou les noms de datastores dans le `global.ds.exclusion.pattern` propriété dans le `scbr.override` fichier de configuration. Voir #Properties you can override,Propriétés que vous pouvez remplacer.

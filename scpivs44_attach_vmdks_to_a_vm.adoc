...
---
= Attachez des VMDK à une VM à l'aide du SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.Étapes
. Dans l’interface utilisateur du client VMware vSphere, sélectionnez *Menu* dans la barre d’outils, puis sélectionnez *Hôtes et clusters* dans la liste déroulante.

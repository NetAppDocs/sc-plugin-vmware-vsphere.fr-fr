...
---
= Désactiver et activer le SnapCenter Plug-in pour VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Connectez-vous à l'interface utilisateur de gestion du SnapCenter Plug-in for VMware vSphere en utilisant le format `\https://<OVA-IP-address>:8080` .  Connectez-vous avec le nom d’utilisateur et le mot de passe administrateur définis au moment du déploiement et le jeton MFA généré à l’aide de la console de maintenance.
+
